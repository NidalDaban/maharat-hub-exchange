@forelse ($users as $user)
    <div class="talent-card">
        ...
        <div class="talent-actions">
            ...
            <form class="invitation-form d-inline" data-username="{{ $user->fullName() }}">
                @csrf
                <input type="hidden" name="destination_user_id" value="{{ $user->id }}">
                <button onclick="validateInvitationEligibility()" type="submit"
                    class="btn btn-outline-primary">دعوة</button>
            </form>
        </div>
    </div>
@empty
